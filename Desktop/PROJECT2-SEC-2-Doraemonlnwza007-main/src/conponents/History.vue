...
<template>
  <div class="bg-teal-900 w-screen h-screen flex flex-col text-font text-white bg-[url('/bg/qq.gif')]">
    <div  class="w-full h-full flex flex-col">
      ...
      <div class=" w-full flex h-[400px] mb-5  overflow-y-auto  text-black bg-white border-8 border-blue-300">
        <table class="w-full ">
          <tr class="text-black bg-white border-b-4 border-blue-300 ">
            <th class="border-r-4 border-blue-300">Name</th>
            <th class="border-r-4 border-blue-300">Hit</th>
            <th class=" border-blue-300">Score</th>
          </tr>
          <tr v-for=" user in datatracking " :key="user.id" class="text-center ">
            <td class="w-3/12 border-r-4 border-blue-300"> {{ user.name != '' ? user.name : "null" }}</td>
            <td class="w-3/12 border-r-4 border-blue-300">{{ user.hit }}</td>
      ...
      <div class=" w-full flex h-[400px] mb-5  overflow-y-auto  text-black bg-white border-8 border-blue-300">
        <table class="w-full">
          <tr class="text-black bg-white   border-b-4 border-blue-300 ">
            <th class="border-r-4 border-blue-300">Name</th>
            <th class="border-r-4 border-blue-300">Hit</th>
            <th class="border-r-4 border-blue-300">Accuracy</th>
            <th class=" border-blue-300">Score</th>
          </tr>
          <tr v-for=" user in dataaimlap " :key="user.id" class="text-center ">
            <td class="w-3/12 border-r-4 border-blue-300"> {{ user.name != '' ? user.name : "null" }}</td>
            <td class="w-3/12 border-r-4 border-blue-300">{{ user.scorehit }}</td>
            <td class="w-3/12 border-r-4 border-blue-300">{{ user.accuracy }}%</td>
            <td class="w-3/12">{{ user.score }}</td>
          </tr>
        </table>
      </div>
